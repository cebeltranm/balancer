...
import { AccountGroupType, AccountType, Period, type Account, type PeriodOption } from '@/types';
...
export function useTotalByCategory() {
    ...
    function get ( category: any, balance: any, period: PeriodOption, displayType: string): CategoryResult {
      ...
      if ( category.children ) {
        children = Object.keys(category.children).map((key) => get(category.children[key], balance, period, displayType) );
        values = children.reduce( (ant, child) => {
            return ant.map( (v, index) => {
              if (CURRENCY?.value && child.data.currency!==CURRENCY.value && child.data.values[index]) {
                const conv = valuesStore.getValue( 
                  getPeriodDate(period.type, index > 0 
              ...
              return {
                ...
                expenses: v.expenses + (child?.data?.values?.[index]?.expenses || 0),
              }
            } );
        }, Array.from(new Array(children?.[0]?.data?.values.length || 0), () => ({ value: 0, in: 0, out: 0, expenses: 0, in_local: 0, out_local: 0 })) );
      }
      ...
      return { 
        ...
        data: {
          name: category.entity && displayType !== 'table' ? category.yahoo_symbol || category.name : category.name,
          fullName: category.entity && displayType !== 'table' ? `${category.entity}::${category.name}` : category.name,
