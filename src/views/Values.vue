<template>
  ...
  <div class="values">
    <DataTable :value="values" @cell-edit-complete="onValueEdit" responsiveLayout="scroll" :resizableColumns="true"
      columnResizeMode="fit" showGridlines class="p-datatable-sm" editMode="cell" :scrollable="true"
      :rowClass="rowPendingSyncClass" scrollHeight="flex" scrollDirection="both" filterDisplay="menu"
      :globalFilterFields="['entity', 'type']" v-model:filters="filters">
      ...
      <Column header="value" style="width:200px" v-tooltip.top="'teest'">
        ...
        <template #editor="{ data }">
          <InputNumber v-model="data.value" mode="currency" :currency="data.currency || Currency.USD"
            :maxFractionDigits="data.currency === Currency.BTC ? 10 : 2" currencyDisplay="code" locale="en-US"
            autofocus />
        </template>
      </Column>
      <Column field="m_m" header="MoM" style="width:80px">
        <template #body="slotProps">
          <div
            :class="{ 'text-right': true, 'text-red-400': slotProps.data.m_m < 0, 'text-green-400': slotProps.data.m_m > 0 }">
      ...
      <Column field="m_m" header="YoY" style="width:80px">
        <template #body="slotProps">
          <div
            :class="{ 'text-right': true, 'text-red-400': slotProps.data.m_y < 0, 'text-green-400': slotProps.data.m_y > 0 }">
...
watch(() => valuesStore.values, () => recalculateValues(), { deep: true })
...
function recalculateValues() {
  ...
  const investments = accounts.filter(a => [AccountGroupType.Investments, AccountGroupType.FixedAssets].includes(accountsStore.getAccountGroupType(a.id)));
  ...
  values.value = [
    ...currencies.map((c) => ({
      ...
      m_m: valuesStore.getValue(prevDate, Currency.USD, c) ? ((valuesStore.getValue(date, Currency.USD, c) - valuesStore.getValue(prevDate, Currency.USD, c)) / valuesStore.getValue(prevDate, Currency.USD, c)) : undefined,
      m_y: valuesStore.getValue(prevYear, Currency.USD, c) ? ((valuesStore.getValue(date, Currency.USD, c) - valuesStore.getValue(prevYear, Currency.USD, c)) / valuesStore.getValue(prevYear, Currency.USD, c)) : undefined
    })),
    ...investments.map((a) => ({
      entity: a.entity,
      id: a.id,
      ...
      m_m: valuesStore.getValue(prevDate, a.id, a.currency) ? ((valuesStore.getValue(date, a.id, a.currency) - valuesStore.getValue(prevDate, a.id, a.currency)) / valuesStore.getValue(prevDate, a.id, a.currency)) : undefined,
      m_y: valuesStore.getValue(prevYear, a.id, a.currency) ? ((valuesStore.getValue(date, a.id, a.currency) - valuesStore.getValue(prevYear, a.id, a.currency)) / valuesStore.getValue(prevYear, a.id, a.currency)) : undefined
    }))
  ];
};
...
async function syncValues() {
  async function process() {
    ...
    const accounts = accountsStore.activeAccounts(getPeriodDate(period.value.type, period.value.value));

    const symbols = accounts
      .filter((a: Account) => [AccountGroupType.Investments, AccountGroupType.FixedAssets].includes(accountsStore.getAccountGroupType(a.id)))
...
async function syncFromYahoo() {
  ...
  if (period.value.value.month === current.month && period.value.value.year === current.year) {
    ...
    const yahoo_symbols = accounts
      .filter(a => [AccountGroupType.Investments, AccountGroupType.FixedAssets].includes(accountsStore.getAccountGroupType(a.id)))
