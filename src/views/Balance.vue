    ...
    <div class="balance">
      ...
      >
        <Column header="Type" style="width: 200px" footer="Net Worth" frozen>
          <template #body="{ data }">
            {{
              [AccountType.Category, "Type"].includes(data.type) ? "Total " : ""
            }}
            {{ data.name }}
          </template>
        </Column>
        <template
          v-for="(title, index) in periodTitles"
          :key="`${title}-${index}`"
        >
          <Column :header="title" style="width: 180px">
            ...
            <template #footer
              ><div class="text-right" style="width: 100%">
                {{ $format.currency(total[index], CURRENCY) }}
              </div></template
            >
          </Column>
          <Column header="&nabla; %" style="width: 80px" v-if="index > 0">
            <template #body="{ data }">
              <div class="text-right" style="width: 100%">
                {{
                  data.values[0] &&
                  Math.abs(data.values[index] / data.values[0]) < 100
                    ? format.percent(
            ...
            <template #footer
              ><div class="text-right" style="width: 100%">
                {{ $format.percent((total[0] - total[index]) / total[index]) }}
              </div></template
            >
          </Column>
        </template>
...
async function recalculateValues() {
  ...
  const totPer = period.value.type === Period.Month ? 13 : 5;
  const totConsPer = period.value.type === Period.Month ? 3 : 5;
  let per: any[] = Array.from(new Array(totConsPer), (_val, index) => ({
    period: increasePeriod(period.value.type, period.value.value, -index),
  }));

  if (period.value.type === Period.Month) {
    per.push({
      period: {
        year: period.value.value.year - 1,
  ...
  values.value = [
    ...
    const total = t.groups.reduce((ant2, t2) => {
          ...
          for (let i = 0; i < per.length; i++) {
            if (balance?.[a]?.[per[i].position]) {
              if (account?.currency !== CURRENCY?.value) {
                all[account.type].values[i] +=
                  balance[a][per[i].position].value *
      ...
      ant2.push(
        ...Object.keys(groups).map((key) => ({ ...groups[key], name: key })),
      );
      ant2.push({
        ...t2,
        values: Object.keys(groups).reduce(
          (v, key) => {
    ...
    ant.push({
      ...t,
      values: total
        .filter((g: any) => g.type === AccountType.Category)
        .reduce(
          (v: any[], group: any) => {
