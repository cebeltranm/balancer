<template>
  <Chart type="line" :data="barData[0]" :options="{ plugins: { legend: { labels: { color: '#ffffff' } } }, scales: { x: {stacked: false}, y: {position: 'left'}, y1: {position: 'right'} }}" /> 
...
import { getCurrentPeriod, increasePeriod, BACKGROUNDS_COLOR_GRAPH } from '@/helpers/options.js';
...
const barData = computed(() => {
    ...
    for (var i = 1 ; i < data.length + 1; i++) {
      labels.push(`${currentPeriod.year}${props.period.type === Period.Month ? '/'+currentPeriod.month: ''}${props.period.type === Period.Quarter ? '/'+currentPeriod.quarter: ''}`);
