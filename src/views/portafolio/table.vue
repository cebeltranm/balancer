  ...
  <div class="investments-table">
    ...
    >
        ...
        <template #body="{ node }">
          <div class="flex flex-row flex-wrap">
            ...
            <div class="align-items-center justify-content-center">
              ...
              <span v-if="node.data.code" class="text-xs"
                ><a
                  :href="'https://finance.yahoo.com/quote/' + node.data.code"
                  target="_blank"
                  >({{ node.data.code }})</a
                ></span
              >
        ...
        <template #body="{ node }">
          <div
            :class="{
              ...
              'w-full': true,
              'text-red-400':
                getInOut(node.data.values[0], node.data.isCategory) < 0,
              'text-green-400':
                getInOut(node.data.values[0], node.data.isCategory) > 0,
            }"
            v-tooltip.bottom="
              'IN: ' +
              $format.currency(
                node.data.values[0].in,
                node.data.currency || CURRENCY,
              ) +
              '\nOUT: ' +
              $format.currency(
                node.data.values[0].out,
                node.data.currency || CURRENCY,
              ) +
              '\nLOCAL IN: ' +
              $format.currency(
                node.data.values[0].in_local,
                node.data.currency || CURRENCY,
              ) +
              '\nLOCAL OUT: ' +
              $format.currency(
                node.data.values[0].out_local,
                node.data.currency || CURRENCY,
              )
            "
          >
            {{
              $format.currency(
                getInOut(node.data.values[0], node.data.isCategory),
                node.data.currency || CURRENCY,
              )
            }}
          </div>
        ...
        <template #footer>
          <div
            :class="{
              'text-right': true,
              'w-full': true,
              'text-red-400': getInOut(total[0]) < 0,
              'text-green-400': getInOut(total[0]) > 0,
            }"
          >
            {{ $format.currency(getInOut(total[0]), CURRENCY) }}
          </div>
        </template>
      </Column>
        ...
        <template #body="{ node }">
          <div
            :class="{
              'text-right': true,
              'w-full': true,
              'text-red-400': node.data.values[0].expenses > 0,
            }"
          >
            {{
              $format.currency(
                node.data.values[0].expenses,
                node.data.currency || CURRENCY,
              )
            }}
          </div>
        ...
        <template #footer>
          <div
            :class="{
              'text-right': true,
              'w-full': true,
              'text-red-400': total[0].expenses > 0,
              'text-green-400': total[0].expenses < 0,
            }"
            v-if="total && total.length > 0"
          >
            {{ $format.currency(total[0].expenses, CURRENCY) }}
          </div>
        </template>
      </Column>
        ...
        <template #body="{ node }">
          <div
            :class="{
              'text-right': true,
              'w-full': true,
              'text-red-400': node.data.values[0].gp < 0,
              'text-green-400': node.data.values[0].gp > 0,
            }"
            v-tooltip.bottom="
              $format.currency(
                node.data.values[0].gp_value,
                node.data.currency || CURRENCY,
              )
            "
          >
        ...
        <template #footer>
          <div
            :class="{
              'text-right': true,
              'w-full': true,
              'text-red-400': total[0].gp < 0,
              'text-green-400': total[0].gp > 0,
            }"
            v-if="total && total.length > 0"
      ...
      <Column header="Balance" style="width: 200px">
        ...
        <template #footer>
          <div
            :class="{ 'text-right': true, 'w-full': true }"
            v-if="total && total.length > 0"
          >
