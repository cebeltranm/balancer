<template>
  <Toolbar>
    <template #start>
      ...
      <AccountsSelector v-model:accounts="selectedAccounts" :groups="[AccountGroupType.Investments]" :date="getPeriodDate(period.type, period.value)"  v-if="displayType === 'bar'"/>
    </template>
    <template #end>
  ...
  <template v-if="storageStore.status.authenticated">
    <table-view v-if="displayType === 'table'" :accountsGrouped="accountsGroupBy" :total="getTotal" />
    <pie-view v-if="displayType === 'pie'" :accountsGrouped="accountsGroupBy" :total="getTotal" :date="getPeriodDate(period.type, period.value)"/>
...
<script lang="ts" setup>
  ...
  import { AccountGroupType, AccountType, Period, type PeriodOption } from '@/types';
  import { getCurrentPeriod, getPeriodDate } from '@/helpers/options.js';
  import { mapInvestmentsBySubCategory, accountsGrupedByAttribute } from '@/helpers/investments';
  ...
  const accountsGroupBy = computed(() => {
      const numPer = displayType.value !== 'bar' ? 2 : (period.value.type === Period.Year ? 10 : period.value.type === Period.Month ? 24 : 16);
      const balance = balanceStore.getBalanceGroupedByPeriods(period.value.type, numPer, period.value.value);
      var inv: any = {};
      if (displayType.value !== 'pie' || typeInvestment.value === 'ByCategory') {
        inv = accountsStore.accountsGroupByCategories([AccountGroupType.Investments], getPeriodDate(period.value.type, period.value.value), period.value.type);
        if(inv?.Investments && displayType.value === 'bar' && selectedAccounts.value?.length > 0 ) {
          inv = {
            Investments: Object.keys(inv.Investments)
              .filter(key => Object.keys(inv.Investments[key].children)
        ...
        return inv?.Investments && Object.keys(inv.Investments).map( (key) => totalByCategory( inv.Investments[key], balance, period.value, displayType.value)) || [];
      }
      const accounts = accountsStore.activeAccounts( getPeriodDate(period.value.type, period.value.value), period.value.type, [AccountGroupType.Investments]);
      ...
      if (['ByRegion', 'ByAssetClass'].includes(typeInvestment.value)) {
        ...
        const data = nestedCategories.map(category => totalByCategory(category, balance, period.value, displayType.value)) || [];
        return data;
      }
      return inv && Object.keys(inv).map( (key) => totalByCategory({
          type: AccountType.Category,
          name: key,
          children: inv[key]
        }, balance, period.value, displayType.value)) || [];
  });

  const getTotal = computed(() => {
    const val1 = accountsGroupBy.value.reduce( (ant, child) => {
      if (!ant) {
