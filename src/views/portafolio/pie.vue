<template>
  ...
  />  
  <TreeTable :value="accountsGrouped"
    ...
    <Column header="Diff"  style="width:200px">
      <template #body="{node}">
        <div :class="{ 'text-right': true, 'w-full': true, 
          'text-red-400': ( total[0].value * node.data.expected) > (node.data.values[0].value * (1 + DIFF_PERCENT)) || ( total[0].value * node.data.expected) < (node.data.values[0].value * (1-DIFF_PERCENT)), 
          }"
          v-if="(node.data.currency || CURRENCY === CURRENCY) && node.data.expected">
          <div>
        {{ $format.currency( ( total[0].value * node.data.expected ) - node.data.values[0].value, node.data.currency || CURRENCY) }}
        </div>
        </div>
      </template>
    </Column> 
...
const treeMap = computed(() => {
  const groupElements = (group: any, parent: string, parentArr: any[]) => {
    const values = group.filter(g => g.data.values[0].value).map( (g: any) => {
      return g.data.currency === CURRENCY.value ? g.data.values[0].value :  g.data.values[0].value * valuesStore.getValue( props.date, g.data.currency, CURRENCY.value);
    });
  ...
  if (onChartReady.value && window?.google?.visualization) {
    var byValue = props.accountsGrouped;
