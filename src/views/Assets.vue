...
<script lang="ts" setup>
  ...
  const byCategory = computed(() => {
      const balance = balanceStore.getBalanceGroupedByPeriods(period.value.type, 1, period.value.value);
      const assets = accountsStore.accountsGroupByCategories([AccountGroupType.Assets, AccountGroupType.AccountsReceivable, AccountGroupType.Liabilities], getPeriodDate(period.value.type, period.value.value));
