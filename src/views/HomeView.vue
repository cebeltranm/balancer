...
const values = computed(() => {
  ...
  if (balance) {
    let types: Record<string, AccountType[]> = {
    ...
    const total = Object.keys(accountsStore.accounts).reduce(
      (ant, k) => {
        const a = accountsStore.accounts[k];
        if (
          balance[a.id] &&
          balance[a.id][currentPeriod.month] &&
          balance[a.id][currentPeriod.month].value
        ) {
