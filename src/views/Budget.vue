  ...
  <div class="expenses">
    ...
    >
      <Column header="Expense" style="width: 200px" footer="Total" frozen>
        <template #body="{ data }">
          {{ data.type !== AccountType.Category ? "&nbsp;>&nbsp;&nbsp;" : "" }}
          {{ data.name }}
        </template>
      </Column>
      ...
      <Column
        v-for="m of months"
        :key="m"
        :header="format.month(m)"
        class="text-right p-0"
        style="width: 150px"
      >
        <template #body="{ data }">
          <div
            class="text-right m-2 p-1 p-overlay-badge"
            @contextmenu="onShowContextMenu($event, data, m)"
          >
            {{
              format.currency(
                data.type === AccountType.Category
            ...
            <Badge
              ...
              "
              class="ml-auto"
              :value="comments[data.id][m]?.length"
              @click.stop.prevent="editComments($event, data, m)"
            />
          </div>
        </template>
        <template #editor="{ data }">
          <InputNumber
            v-if="data.type !== AccountType.Category"
            v-model="data[m]"
            mode="currency"
    ...
    <CommentsDialog
      ref="commentDialog"
      @update:modelValue="addComments($event)"
    ></CommentsDialog>
  </div>
</template>
...
const contextMenuItems = [
  {
    ...
    icon: "pi pi-play",
    command: () => {
      for (let i = 1 + Number(selecteMenuData.value.month); i < 13; i++) {
        selecteMenuData.value.data[i] =
          selecteMenuData.value.data[selecteMenuData.value.month];
      }
    },
  },
  {
  ...
  },
  {
    ...
    command: () => {
      commentDialog.value.show(
        (comments.value &&
          selecteMenuData.value &&
          selecteMenuData.value.data &&
          comments.value[selecteMenuData.value.data.id] &&
          comments.value[selecteMenuData.value.data.id][
            selecteMenuData.value.month
          ]) ||
          [],
      );
    },
...
watch(
  () => budgetStore.budget[period.value.value.year],
  () => recalculateValues(),
  { deep: true },
);
...
async function recalculateValues() {
  ...
  const budget = await budgetStore.loadBudgetForYear(
  ...
  const getByCategory = (category: string, sub: string, accounts: any) => {
    return Object.keys(accounts).reduce((ant: any[], a) => {
      ...
      if (account.type === AccountType.Category) {
        ant.push(
          ...getByCategory(category, `${account.name} / `, account.children),
        );
      } else {
        ant.push({
          type: account.type,
          ...
          ...months.reduce((ant, m) => {
            ant[m] =
              (budget && budget[account.id] && budget[account.id][m]) || null;
            return ant;
          }, {}),
        });
      }
  ...
  if (accounts[AccountGroupType.Expenses]) {
    res = Object.keys(accounts[AccountGroupType.Expenses]).reduce(
      (ant: any, a) => {
...
function getTotal(data, month) {
  ...
  const date = getPeriodDate(period.value.type, period.value.value);
  let vfiltered = values.value.filter((v) => v.type !== AccountType.Category);
  ...
  return valueStore.joinValues(
    date,
    (data && data.currency) || CURRENCY?.value,
    totals,
  );
}

function getRowClass(data: any) {
  return data.type === AccountType.Category
    ? "bg-blue-900"
    : rowPendingSyncClass(data);
}
...
function addComments(comment: any) {
  if (comment) {
    if (comment.length > 0) {
      ...
      comments.value[selecteMenuData.value.data.id] &&
      comments.value[selecteMenuData.value.data.id][selecteMenuData.value.month]
    ) {
      delete comments.value[selecteMenuData.value.data.id][
        selecteMenuData.value.month
      ];
    }
