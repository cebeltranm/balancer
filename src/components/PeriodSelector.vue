<template>
  ...
  <Select
    ...
    panelClass="z-5"
    v-if="!props.onlyPeriod"
    @update:model-value="onChange"
  />
  <div
    class="p-2 w-12rem text-center text-xl font-bold"
    v-if="props.onlyPeriod"
  >
...
const periodTypes = computed(() => {
  ...
  const quarter = props.onlyType
    ? "Quarter"
    : `${selectedPeriod.value.year} / Q${selectedPeriod.value.quarter}`;
  const month = props.onlyType
    ? "Month"
    : `${selectedPeriod.value.year} / ${format.month(selectedPeriod.value.month)}`;
...
const canIncrease = computed(() => {
  ...
  return !(
    selectedPeriod.value.year >= current.year &&
    (typePeriod.value === Period.Year ||
      (typePeriod.value === Period.Month &&
        selectedPeriod.value.month >= current.month) ||
      (typePeriod.value === Period.Quarter &&
        selectedPeriod.value.quarter >= current.quarter))
  );
});

function changePeriod(periods: number) {
  selectedPeriod.value = increasePeriod(
    typePeriod.value,
