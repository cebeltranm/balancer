...
describe('transactions store', () => {
  ...
  it('loads month transactions and merges pending entries', async () => {
    ...
    vi.mocked(idb.getAllTransactions).mockResolvedValue([
      ...
      { id: 3, date: '2025-02-20', description: 'C', values: [], deleted: false },
      { id: 4, date: '2025-02-01', description: 'D', values: [], deleted: true },
    ] as any);
  ...
  it('marks transaction as deleted when deleting', async () => {
    const store = useTransactionsStore();
    await store.deleteTransaction({ id: 99, date: '2025-02-20', description: 'x', values: [] } as any);

    expect(idb.saveTransaction).toHaveBeenCalledWith(expect.objectContaining({ id: 99, deleted: true }));
    expect(updatePendingToSync).toHaveBeenCalled();
  });
});
