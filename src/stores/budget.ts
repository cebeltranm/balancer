...
export const useBudgetStore = defineStore("budget", () => {
  ...
  function getBudgetGrupedByPeriod(
    type: Period,
    numPeriods: number,
    params: PeriodParams,
  ) {
    const groupedBudget: GroupedBudgetData = groupDataByPeriods<number>(
      ...
      (key: string, year: number, months: number[]) =>
        months
  ...
  async function loadBudgetForYear(
    year: number,
    reload: boolean,
  ): Promise<YearlyBudgetData> {
  ...
  async function setBudgetForYear(
    year: number,
