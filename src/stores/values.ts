...
export const useValuesStore = defineStore('values', () => {
    ...
    function getValue(date: Date, asset: string, currency: string, maxLevels: number = 3): number {
        if (asset === currency) {
            return 1;
        }

        let currentPeriod: PeriodParams = { year: date.getFullYear(), month: date.getMonth() + 1, quarter: 1 };
    ...
    function joinValues(date: Date, currency: string, values: [{value: number, asset: string, entity?:string }]) {
        return values.reduce ( (ant: number, v: any) => {
          return ant + (
            v.asset === currency ? v.value : v.value * getValue(date, v.asset, currency)
          );
        }, 0 )
    }

    async function setValuesForMonth(year: number, month: number, newValue: valueData) {
