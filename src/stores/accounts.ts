...
export const useAccountsStore = defineStore('accounts', () => {
    ...
    function activeAccounts(date: Date, period: Period = Period.Month, groupTypes?: AccountGroupType[]): Account[] {
        return Object.keys(accounts.value)
            .filter((a: string) => {
                const fromYear = accounts.value[a]?.activeFrom?.getFullYear();
                const fromMonth = (period === Period.Month ? accounts.value[a]?.activeFrom?.getMonth() || 0 : 0);

                const toYear = accounts.value[a]?.hideSince?.getFullYear();
                const toMonth = (period === Period.Month ? accounts.value[a]?.hideSince?.getMonth() || 12 : 12);
            ...
            .filter((account: any) => !groupTypes || groupTypes.includes( getAccountGroupType(account) ))
            .map((a: string) => accounts.value[a]);
    }

    const accountsGroupByCategories = (groupTypes?: AccountGroupType[], date: Date = new Date(), period: Period = Period.Month) => {
        return activeAccounts(date, period, groupTypes)
    ...
    const getAccountGroupType = (id: string): AccountGroupType | null => {
        return id && accounts.value[id] && Object.keys(ACCOUNT_GROUP_TYPES).find((k: string) => ACCOUNT_GROUP_TYPES[k].includes(accounts.value[id].type))
    }
    ...
    function isAccountInUnits(id: string){ 
        if (id && accounts.value[id]) {
            return [AccountType.ETF, AccountType.Stock, AccountType.Crypto].includes( accounts.value[id].type );
        }
        return false;
    }
    ...
    async function loadAccounts(reload: boolean) {
        if (!reload && accounts.value.length > 0) {
        ...
        const nAccounts = Object.keys(lAccounts).filter(id => id != 'default').reduce((ant: any, id: string) => {
            ant[id] = {
                ...lAccounts[id],
                activeFrom: lAccounts[id].activeFrom && new Date(lAccounts[id].activeFrom),
                hideSince: lAccounts[id].hideSince && new Date(lAccounts[id].hideSince),
                id
            }
            return ant;
        }, {});
    ...
    return { accounts, loadAccounts, expensesByCategories, isAccountInUnits, getAccountGroupType, accountsGroupByCategories, activeAccounts, getAccountFullName };
});