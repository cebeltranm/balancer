...
export const useStorageStore = defineStore('storage', () => {
    ...
    const status = ref({ inSync: false, offline: false, loggedIn: false, authenticated: false })
        ...
        try {
            ...
            if (trans) {
                ...
                await Promise.all(trans.map( t => {
                  return transactionsStore.loadTransactionsForMonth(t.year, t.month, false);
                }));
                ...
                await balanceStore.recalculateBalance(firstMonth.year, firstMonth.month, true);
            }
    ...
    async function updatePendingToSync() {
        ...
        if ((pendingToSync.value.transactions !== transactions && transactions) ||
    ...
    async function executeInSync(promise: Promise<any>) {
        try {
            currentSyncPromise && await currentSyncPromise;
        } catch (e) { }
