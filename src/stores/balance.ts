...
import { Period, type PeriodParams, type BalanceEntry, AccountGroupType, AccountType, type Transaction } from '@/types';
...
export const useBalanceStore = defineStore('balance', () => {
    ...
    function getBalanceGroupedByPeriods(
        ...
        return groupDataByPeriods<BalanceEntry>(
            ...
            (accountId: string, year: number, months: number[]) => months
                .map(monthNum => balance.value[year]?.[accountId]?.[monthNum] as BalanceEntry | undefined || createEmptyBalanceEntry())
                .reduce((accumulated, monthValue) => {
                    ...
                    if (account && [AccountType.Expense, AccountType.Income].includes(account.type)) {
                        result.value = accumulated.value + (monthValue?.value ?? 0);
                    } else {
                        result.value = monthValue?.value ?? accumulated.value;
                    }

                    if (accountsStore.getAccountGroupType(accountId) === AccountGroupType.Investments) {
                        const investmentFields = ['in', 'out', 'in_local', 'out_local', 'expenses'] as const;
                        investmentFields.forEach(field => {
                            result[field] = accumulated[field] + (monthValue?.[field] ?? 0);
                        });
                    }
                    if (account && [AccountType.ETF, AccountType.Stock, AccountType.Crypto].includes(account.type)) {
     ...
     */
    async function loadBalanceForYear(year: number, reload = false): Promise<YearlyBalanceData | null> {
    ...
    async function recalculateBalance(year: number, month: number, save: boolean): Promise<YearlyBalanceData> {
        const JANUARY = 1;
        const DECEMBER = 12;
        ...
        const prevBalance = month === JANUARY ? await loadBalanceForYear(year - 1, false) : yearBalance;
        const prevMonth = month === JANUARY ? DECEMBER : month - 1;
        const monthDate = new Date(year, month - 1, 1);
        
        const transactions = await transactionsStore.loadTransactionsForMonth(year, month);
        ...
        if (transactions) {
            for (const transaction of transactions) {
                for (const value of transaction.values) {
                    ...
                    accountValueChanges[value.accountId] = (accountValueChanges[value.accountId] || 0) + accountValue;
                }
            }
        }
        ...
        if (transactions) {
            const investmentTransactions = transactions.filter((t: Transaction) => 
                t.values.some((v: any) => accountsStore.getAccountGroupType(v.accountId) === AccountGroupType.Investments)
            );
            
            for (const transaction of investmentTransactions) {
                const investmentValues = transaction.values.filter((v: any) => 
                    accountsStore.getAccountGroupType(v.accountId) === AccountGroupType.Investments
                );
                ...
                for (let invIndex = 0; invIndex < investmentValues.length; invIndex++) {
                    ...
                    if (!investmentData[accountId]) {
                        investmentData[accountId] = {
                            in: 0, out: 0, in_local: 0, out_local: 0, expenses: 0, units: 0
                        };
                    }
                    ...
                    for (const transactionValue of transaction.values) {
                        const valueGroupType = accountsStore.getAccountGroupType(transactionValue.accountId);
                        const transactionValueAmount = transactionValue.accountValue || 0;
                        
                        // Handle expenses (only count once per transaction)
                        if (valueGroupType === AccountGroupType.Expenses && invIndex === 0) {
                        ...
                        // Handle investment flows (exclude self-transfers)
                            ...
                            const isSameEntity = valueGroupType === AccountGroupType.Investments && 
                                accounts[accountId]?.entity && 
                                accounts[transactionValue.accountId]?.entity && 
                                accounts[accountId].entity === accounts[transactionValue.accountId].entity;
        ...
        for (const accountKey of Object.keys(accounts)) {
            ...
            switch (account.type) {
                ...
                case AccountType.AccountPayable:
                    const previousValue = prevBalance?.[accountId]?.[prevMonth]?.value || 0;
                ...
                case AccountType.MutualFund:
                    ...
                    monthlyBalance.value = valuesStore.getValue(monthDate, accountId, account.currency) || 0;
                    break;
                ...
                case AccountType.Crypto:
                    ...
                    const previousUnits = prevBalance?.[accountId]?.[prevMonth]?.units || 0;
                    ...
                    monthlyBalance.value = totalUnits * (valuesStore.getValue(monthDate, accountId, account.currency) || 0);
                    break;
            }
        }
        ...
        const shouldRecalculateNext = year < currentPeriod.year || (year === currentPeriod.year && month < currentPeriod.month);
        ...
        const shouldSave = save && (month === DECEMBER || (month === currentPeriod.month && year === currentPeriod.year));
        if (shouldSave) {
            idb.saveJsonFile({
