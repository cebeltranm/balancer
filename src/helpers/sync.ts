...
export async function syncTransactions() {
  ...
  if (transactions.length > 0) {
    ...
    const res = await Promise.all(
      Object.keys(byMonth).map(async (year) => {
        return Promise.all(
          Object.keys(byMonth[year]).map(async (month) => {
            const file = await files.readJsonFile(
              `transactions_${year}_${month}.json`,
              false,
            );
            let trans = byMonth[year][month];
            if (file) {
              trans = file.filter(
                (t: any) => !transactions.find((t2) => t2.id === t.id),
              );
...
export async function getAllFilesInCache() {
  ...
  return fileKeys.reduce(
    async (ant, fileName) => {
      const data = await ant;
      const key = String(fileName);
      const file = await idb.getJsonFile(key);
      data[key] = file.date_cached;
      return data;
    },
