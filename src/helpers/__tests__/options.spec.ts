...
describe('options helper', () => {
  ...
  it('increases month/quarter/year periods', () => {
    expect(increasePeriod(Period.Month, { year: 2025, month: 1, quarter: 1 }, -1)).toEqual({
      year: 2024,
      month: 12,
      quarter: 4,
    });

    expect(increasePeriod(Period.Quarter, { year: 2025, month: 3, quarter: 1 }, 1)).toEqual({
      year: 2025,
      quarter: 2,
      month: 6,
    });

    expect(increasePeriod(Period.Year, { year: 2024, month: 12, quarter: 4 }, 1)).toEqual({
      year: 2025,
      quarter: 1,
      month: 2,
    });
  ...
  it('builds labels, options, and period dates', () => {
    expect(periodLabel(Period.Quarter, { year: 2025, month: 6, quarter: 2 })).toBe('2025 / Q2');
    ...
    const pastMonthDate = getPeriodDate(Period.Month, { year: 2024, month: 12, quarter: 4 });
    expect(pastMonthDate.getDate()).toBe(2);

    const currentMonthDate = getPeriodDate(Period.Month, { year: 2025, month: 2, quarter: 1 });
