...
type groupType = Record<
  string,
  { type: string; name: string; expected: number; children: any }
>;
  ...
  Object.keys(subClassObj).forEach((id) => {
    ...
    if (!grouped[item.type]) {
      grouped[item.type] = {
        ...
        name: item.type,
        expected:
          (expected && expected[item.type] && expected[item.type].value) || 0,
        children: {},
      };
    }
...
export function mapInvestmentsBySubCategory(inv: any, expected: any): any[] {
  ...
  return Object.keys(inv).map((assetClass) => ({
    ...
    children: Object.keys(inv[assetClass]).map((subClass) => ({
      ...
      expected:
        (expected[assetClass] &&
          expected[assetClass][subClass] &&
          expected[assetClass][subClass].value) ||
        0,
      children: groupInvestmentsByType(
        inv[assetClass][subClass],
        expected[assetClass] && expected[assetClass][subClass],
      ),
    })),
  }));
}

export function accountsGrupedByAttribute(
  accounts: Account[],
  att: string,
  def: any | undefined,
) {
